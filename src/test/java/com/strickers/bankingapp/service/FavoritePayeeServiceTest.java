...
public class FavoritePayeeServiceTest {
	...
	public void testAddFavoritePayee() throws MaximumFavoriteReachedException, CustomerNotExistException,
		...
		Mockito.when(favoritePayeeService.validateAccount(2356L)).thenReturn(true);
	...
	@Test(expected = BankNotExistException.class)
	...
	public void testAddFavoritePayeePositive() throws MaximumFavoriteReachedException, CustomerNotExistException,
		...
		favoritePayeeResponseDto.setStatusCode(StringConstant.SUCCESS_STATUS);
		favoritePayeeResponseDto.setMessage(StringConstant.PAYEE_ADDED);
		favoritePayeeRequestDto.setAccountNumber(123L);
		favoritePayeeRequestDto.setFavoriteName("Jaanu");
		favoritePayeeRequestDto.setIfscCode("ICIC123");
		favoritePayees = null;

		Mockito.when(customerRepository.findByCustomerId(1)).thenReturn(customer);
		Mockito.when(favoritePayeeRepository.findByCustomerIdAndAccountNumber(1, 123L)).thenReturn(favoritePayees);
		Mockito.when(favoritePayeeRepository.getPayeesByCustomerIdAndStatus(1, StringConstant.ACTIVE_STATUS))
				.thenReturn(favoritePayeeList);
		Mockito.when(bankRepository.findByIfscCode("ICIC123")).thenReturn(bank);
		Mockito.when(favoritePayeeRepository.save(favoritePayee)).thenReturn(favoritePayee1);
		Integer favoritePayeeResponseDtos = favoritePayeeService
				.addFavoritePayee(customer.getCustomerId(), favoritePayeeRequestDto).getStatusCode();
		assertEquals(StringConstant.SUCCESS_STATUS, favoritePayeeResponseDtos);
